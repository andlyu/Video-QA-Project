...
class InputUnitVisual(nn.Module):
    def __init__(self, k_max_frame_level, k_max_clip_level, spl_resolution, vision_dim, module_dim=512):
        ...
        self.mid_level_motion_cond = CRN(module_dim, k_max_frame_level, k_max_frame_level, gating=False, spl_resolution=spl_resolution)
        self.mid_level_question_cond = CRN(module_dim, k_max_frame_level-2, k_max_frame_level-2, gating=True, spl_resolution=spl_resolution)
...
class InputUnitVisual(nn.Module):
    def __init__(self, k_max_frame_level, k_max_clip_level, spl_resolution, vision_dim, module_dim=512):
        ...
        self.sequence_encoder = nn.LSTM(vision_dim, module_dim, batch_first=True, bidirectional=False)
        self.mid_sequence_encoder = nn.LSTM(vision_dim, module_dim, batch_first=True, bidirectional=False)

        self.clip_level_motion_proj = nn.Linear(vision_dim, module_dim)
        self.mid_level_motion_proj = nn.Linear(module_dim, module_dim)
        self.video_level_motion_proj = nn.Linear(module_dim, module_dim)
        self.appearance_feat_proj = nn.Linear(vision_dim, module_dim)
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding):
        ...
        mid_level_crn_outputs = []
        for i in range(4): #Second layer [1-6, 7-12, 13-18, 19-24]

            # Enocde mid level motion
            #clip_level_motion = motion_video_feat[:, i, :]  # (bz, 2048)
            _, (mid_level_motion, _) = self.mid_sequence_encoder(motion_video_feat[:, 6*i:6*(i+1),:])
            mid_level_motion = mid_level_motion.transpose(0, 1)
            mid_level_motion_proj = self.mid_level_motion_proj(mid_level_motion)

            # mid level CRNs

            clip_level_crn_motion = self.clip_level_motion_cond(clip_level_crn_outputs[i*6:(i+1)*6], mid_level_motion_proj)
            mid_level_crn_question = self.clip_level_question_cond(clip_level_crn_motion, 
                                                                question_embedding_proj.unsqueeze(1))

            mid_level_crn_output = torch.cat(
                [frame_relation.unsqueeze(1) for frame_relation in mid_level_crn_question],
                dim=1)
            mid_level_crn_output = mid_level_crn_output.view(batch_size, -1, self.module_dim)
            mid_level_crn_outputs.append(mid_level_crn_output)
        ...
        # video level CRNs
        video_level_crn_motion = self.video_level_motion_cond(mid_level_crn_outputs, video_level_motion_feat_proj)
        ...
        return video_level_crn_output, 0
...
class HCRNNetwork(nn.Module):
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, question,
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            visual_embedding, d1_mean = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding)
