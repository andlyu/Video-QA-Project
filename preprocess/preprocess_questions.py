...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_pt', type=str, default=None)#'data/{}/{}_{}_questions.pt')
    parser.add_argument('--vocab_json', type=str, default=None)# ='data/{}/{}_vocab.json')
    ...
    if args.dataset == 'tgif-qa': # TODO: PATH
        if args.mode == 'train':   
            ...
            args.annotation_file = '~/agqa/storage/questions_baseline_gen_captions/Train_frameqa_question-balanced.csv'# % args.metric
            print('arg metric:', args.metric)
        elif args.mode == 'test':
            #args.annotation_file = '~/storage/questions_csv/Test_frameqa_question-%s.csv' % args.metric
            args.annotation_file = '~/agqa/storage/questions_baseline_gen_captions/Test_frameqa_question-balanced.csv'# % args.metric
        else:
            print("invalid mode", args.mode)
        args.output_pt = '/usr0/home/alyubovs/agqa/storage/questions_baseline_gen_captions/tgif-qa_{}_{}_questions-%s.pt' % args.metric
        args.vocab_json = '/usr0/home/alyubovs/agqa/storage/questions_baseline_gen_captions/tgif-qa_frameqa_vocab-balanced.json'

        # check if data folder exists
        if not os.path.exists('data/tgif-qa/{}'.format(args.question_type)):
            print('path not found', 'data/tgif-qa/{}'.format(args.question_type))
